...
5. If memory tools are available, use `memory_search` to rebuild recent continuity before answering questions about prior work, decisions, dates, prefs, or todos
...
- At the end of any substantial work session, write a short summary to `memory/YYYY-MM-DD.md` unless an equivalent summary was already saved during the session
...
- Default to **inline work in the main session** for implementation tasks when reliability and user feedback matter.
- Treat ACP as **experimental / secondary** until it proves equal or better than inline execution for real builds in this workspace.
- Only prefer ACP when it provides a clear advantage (isolation, long-running coding session, parallel branch of work) **and** progress can be monitored usefully.
- For major coding tasks, inline-first is the default unless the user explicitly asks for ACP or the ACP workflow has already been verified for that task type.
- Do not silently switch major implementation work from inline to ACP just because ACP is available.
